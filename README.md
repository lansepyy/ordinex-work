...
### v2.7.2 (2026-04-26)
- **新增**：任务创建弹窗新增「启用实时监控」开关，仅本地路径任务生效；开启后会轮询监听源路径中的新增/变更视频文件，并自动触发任务运行
- **新增**：任务创建弹窗新增「启用定时运行」独立开关；关闭时仅保存定时配置但不生效，开启后才参与后台调度
- **优化**：实时监控与定时调度可同时开启，两个触发源互不冲突；运行中、暂停中或已入队任务会自动跳过重复触发，避免重复执行
- **说明更新**：README 补充任务调度的新行为说明，版本号与 Web 页面版本同步更新

### v2.7.1 (2026-04-24)
- **文档更新**：明确“新识别引擎”并非仅用于文件管理，任务管理（本地/云）、文件管理、识别测试三处入口均可触发
- **文档更新**：补充新引擎触发前提——需开启 `use_new_recognition_engine`、AI 配置可用（`ai_mode != off` 且有 API Key）、且当前识别进入 AI 兜底分支（如 TMDB 失败或低于阈值）
- **文档更新**：补充识别缓存开关作用范围——`engine.cache:hit` 与文件管理“写入缓存”依赖该开关；历史页“同步反馈学习”不依赖该开关
- **说明修订**：统一“并非每个文件都会走新引擎，而是满足条件时才调用”的行为描述，避免误解

### v2.7.0 (2026-04-21)
- **UI 焕新**：首页「序界」标题全面升级——草书字体（Ma Shan Zheng）+ 蓝紫粉炫酷渐变色 + 呼吸光效动画，透明毛玻璃背景
- **UI 焕新**：三大功能卡片（任务调度 / 多模态识别 / 文件管理）精简瘦身，高度从 180px 缩至 125px，图标从 110px 缩至 76px，紧凑不突兀
- **UI 焕新**：最近任务面板升级为蓝紫渐变底纹 + 渐变描边，每项任务状态改为多色流光渐变（已完成=绿→青 / 运行中=青→蓝→紫→粉 / 暂停=紫→粉 / 待处理=琥珀金→红 / 失败=玫红），配合发光投影
- **UI 焕新**：全局主文字色从纯黑 `#0f172a` 调整为深海军蓝 `#1e3a5f`，次级文字同步调整为蓝灰色调，视觉更舒适统一
- **优化**：TMDB 年份分值算法重构——完全匹配=4分，±1年=3.5分（几乎不影响识别结果），±2年=2分，≥3年渐进归零，配合乘法惩罚双重保障
- **修复**：「何以笙X默」误识别为「X」(2022) 的极端案例——单字符子串匹配得分从 4.0 压缩至按长度比例计算，年份差 7 年时置信度从 7.0 暴降至 0.1，彻底杜绝此类误识别
- **修复**：TMDB 结果短标题防御性检查——当 TMDB 返回标题 ≤ 2 字符且源文件名远长于标题时，自动降低置信度并记录 WARNING 日志（`file_organizer.py` 和 `task_manager.py` 双路覆盖）

### v2.6.0 (2026-04-20)
- **新增**：文件管理识别重命名逻辑重构——文件夹和文件命名统一遵循系统设置中的电影/电视剧命名模板（文件夹取模板路径第一段，文件取最后一段），不再使用硬编码规则
- **新增**：Docker 镜像双标签构建——本地构建脚本（`build.sh` / `build.bat`）和 CI 流水线同时支持 `latest` + 版本号标签（含 major.minor 补充标签）
- **新增**：GitHub Actions 手动构建支持——可通过 `workflow_dispatch` 手动触发构建，支持自定义版本号、额外镜像标签、是否推送
- **新增**：Web UI 识别增强模块总开关——将 `use_new_recognition_engine` 主开关暴露到「识别增强」设置页面顶部，开启后子组件才生效，关闭时子面板视觉置灰
- **新增**：识别增强参数警告提示——在增强模块配置区顶部添加醒目警告，提示用户不了解参数作用时建议保持默认配置不要修改
- **修复**：CI 版本提取改为从 `web_server.py` 的 `APP_VERSION` 常量读取，避免从 HTML 模板变量中获取到未渲染的占位符
